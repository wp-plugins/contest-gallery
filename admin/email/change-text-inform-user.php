...
add_action('wp_enqueue_scripts','cg_css_change_text_inform_user');
	...
	//Check all GET Variables
		...
		if (isset($_GET["informuser"])) {
		  	$muster = "/^[a-zA-Z0-9-_.]+$/"; // reg. Ausdruck für Zahlen und Wörter
		  if (preg_match($muster, $_GET["informuser"]) == 0 AND $_GET["informuser"] != 'true') {
			die('Manipulieren Sie die URL nicht!');
		  } else {
			$informuser = $_GET["informuser"];
		  }
		}
	...
	if ($_POST['inform']==false AND $_POST['informUserText']==true) {
...
//--------------------------------- ANFANG FORMULAR --------------------------------------------	
		...
		// Formular Anfang
		echo "<form action='?page=contest-gal1ery/index.php&option_id=$GalleryID&inform_user=$GalleryID' method='post'>";
		
		// Main Div
		echo '<div style="display:table;width:914px !important;border: 1px solid #DFDFDF;padding:10px;background-color:#fff;font-size:16px;">';
		...
		echo '<input type="text" name="url" value="'.$selectSQLemail->URL.'" size="119" maxlength="110" '.$AllowGalleryScript.' ><br/>';		
		//echo $inputUrlLink;
